...
class DenseFeatureReader(object):
    def __init__(self, data_io_param):
        ...
        self.ts_idx = data_io_param.ts_idx #None
    ...
    def gen_output_format(features, data_type='float', exclusive_data_type_fid_map=None,
        ...
        if ts_idx !=None:
            if start_date == None:
                origin_time = datetime(1970,1,1)
            else:
                origin_time = datetime.striptime(start_data, "%Y-%m-%d")
        ...
        if output_format == "dense":
            ...
            if data_type in ["int", "int64", "long", "float", "float64", "double"]:
                for i in range(len(features)):
                    ...
                    if i == ts_idx:
                        if (features[i].isnumeric()):
                            continue
                        else:
                            time_obj = datetime.strptime(features[i], '%Y-%m-%d %H:%M:%S')
                            res = int((time_obj - origin_time).total_seconds())
                            format_features[i] = res
